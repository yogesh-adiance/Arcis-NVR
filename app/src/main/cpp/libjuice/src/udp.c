...
static socket_t create_socket_for_addrinfo(const udp_socket_config_t *config,
	...
	// Request 8 MiB UDP socket buffers (was 1 MiB) for big-frame burst tolerance.
	//
	// A single 4 MB H.265 / HEVC keyframe is fragmented into ~3870 UDP datagrams
	// of 1100 bytes each (see consumer_api.c and provider_configurable.c).  At a
	// 1 MiB rx buffer the kernel could only hold ~1000 of those before dropping
	// the rest, and one missing chunk discards the whole frame on the consumer
	// side (no retransmit, no FEC).  8 MiB absorbs one full keyframe plus
	// headroom and is the single largest contributor to "stream stutters on big
	// frames after a few seconds".
	//
	// The kernel silently caps SO_RCVBUF/SO_SNDBUF at /proc/sys/net/core/rmem_max
	// and wmem_max.  On Linux servers running with CAP_NET_ADMIN we additionally
	// try the *FORCE variants which bypass that cap.  On Android the regular
	// setsockopt is honoured up to the device's system limit.
	const sockopt_t buffer_size = 8 * 1024 * 1024;
	...
	setsockopt(sock, SOL_SOCKET, SO_RCVBUFFORCE, (const char *)&buffer_size, sizeof(buffer_size));
#endif
#ifdef SO_SNDBUFFORCE
	setsockopt(sock, SOL_SOCKET, SO_SNDBUFFORCE, (const char *)&buffer_size, sizeof(buffer_size));
#endif
...
int udp_sendto(socket_t sock, const char *data, size_t size, const addr_record_t *dst) {
	...
	if (getsockname(sock, (struct sockaddr *)&name.addr, &name.len) == 0) {
		if (name.addr.ss_family == AF_INET6 && tmp.addr.ss_family == AF_INET)
